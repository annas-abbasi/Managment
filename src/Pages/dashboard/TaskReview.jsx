    ...
    if (!timers[taskId].ended && !timers[taskId].intervalId) {
      const intervalId = setInterval(() => {
        setTimers(prevTimers => ({
          ...
          [taskId]: {
            ...prevTimers[taskId],
            time: prevTimers[taskId].time + 1
          }
        }));
      }, 1000);
      setTimers(prevTimers => ({
        ...prevTimers,
        [taskId]: { ...prevTimers[taskId], intervalId }
      }));
    }
  };
              ...
              {tasks.map((task) => (
                <tr key={task._id} className='border-b border-gray-100'>
                  ...
                  <td className="whitespace-no-wrap py-4 text-sm font-normal text-gray-500 sm:px-6 lg:table-cell flex flex-col gap-3 space-y-2 border">
                    <button
                      className='py-2 px-4 min-w-16 w-full rounded-md bg-green-600 text-white font-semibold'
                      onClick={() => handleStart(task._id)}
                      disabled={timers[task._id]?.ended}
                    >
                      Start
                    </button>
                    <button
                      className='py-2 px-4 min-w-16 w-full rounded-md bg-blue-600 text-white font-semibold'
                    ...
                    <button
                      className='py-2 px-4 min-w-16 w-full rounded-md bg-red-600 text-white font-semibold'
